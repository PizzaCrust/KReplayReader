...
/**
 * Allows to handle a chunk manually.
 */
interface ChunkHandler {
    fun handle(values: List<Any>, chunk: ByteChunk)
}
...
fun ByteChunk.deserialize(clazz: KClass<*>): Any {
    ...
    constructor.parameters.forEach {
        ...
        if (it.type.classifier !is KClass<*>) throw UnsupportedOperationException("Classifier is not KClass")
        val parameterClass = it.type.classifier as KClass<*>
        if (ChunkHandler::class.java.isAssignableFrom(parameterClass.java)) {
            val handler = parameterClass.createInstance() as ChunkHandler
            handler.handle(values, this)
            values.add(handler)
        } else {
            when (parameterClass) {
                ...
                Boolean::class -> values.add(boolean)
                Long::class -> values.add(long)
                SerializableGuid::class -> values.add(SerializableGuid(guid()))
                else -> {
                    if (!parameterClass.hasAnnotation<ChunkSerializable>()) {
...
fun main() {
    ...
    println(ByteChunk(File("season12.replay").readBytes()).deserialize<UnrealReplay>())
}